...
- Instala las dependencias (desde la carpeta `semana-7`):

```powershell
npm install
```

- Si no has instalado `vitest` localmente, ejecuta:
...
Nota: el script `test` está configurado para usar `vitest`. Si prefieres usar `react-scripts test`, modifica el `package.json` en consecuencia.
